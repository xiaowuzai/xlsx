...
// Copy a new DataValidation to a Sheet
func (sh *Sheet) CopyDataValidation(val *DataValidation, ref string) *DataValidation {
	var newDataValidation DataValidation
	newDataValidation = *val
	if len(ref) != 0 {
		newDataValidation.Sqref = ref
	}
	sh.DataValidations = append(sh.DataValidations, &newDataValidation)
	sh.DataValidationCount++
	return sh.DataValidations[sh.DataValidationCount-1]
}

func (sh *Sheet) AddDataValidation(val *DataValidation, ref string) {
	var newDataValidation DataValidation
	newDataValidation = *val
	if len(ref) != 0 {
		newDataValidation.Sqref = ref
...
func (sh *Sheet) ClearDataValidation() {
	sh.DataValidations = make([]*DataValidation, 0)
	sh.DataValidationCount = 0
	return
}
...
func (s *Sheet) makeXLSXSheet(refTable *RefTable, styles *xlsxStyleSheet) *xlsxWorksheet {
	...
	if s.DataValidationCount > 0 {
		worksheet.DataValidations = &xlsxDataValidations{DataValidation: []xlsxDataValidation{}, Count: s.DataValidationCount}
		for _, val := range s.DataValidations {
			xVal := xlsxDataValidation{
				Type:             val.Type,
				AllowBlank:       val.AllowBlank,
				ShowInputMessage: val.ShowInputMessage,
				ShowErrorMessage: val.ShowErrorMessage,
				Sqref:            val.Sqref,
				Formula1:         val.Formula1,
			}
			if len(val.Formula2) != 0 {
				xVal.Formula2 = val.Formula2
			}
			worksheet.DataValidations.DataValidation = append(worksheet.DataValidations.DataValidation, xVal)
		}
	}
