...
func (f *File) makeWorkbook() xlsxWorkbook {
	names := make([]xlsxDefinedName, 0)
	for _, name := range f.DefinedNames {
...
type XlsxDefinedNamePositions struct {
	Name      string
	StartRow  int
	EndRow    int
	StartCol  int
...
func (dn *XlsxDefinedNamePositions) Inside(row, col int) bool {
	if Iabs(dn.CenterRow-row) <= dn.EndRow-dn.CenterRow && Iabs(dn.CenterCol-col) <= dn.EndCol-dn.CenterCol {
		return true
	}
	return false
}

func (f *File) DecodeDefinedNames(sheetName string) ([]*XlsxDefinedNamePositions, error) {
	ret := make([]*XlsxDefinedNamePositions, 0)
	reg := regexp.MustCompile(`\$[A-Z0-9]+`)
	for _, definedName := range f.DefinedNames {
		if !strings.Contains(definedName.Name, "Down_") {
			continue
		}
		split := strings.Split(definedName.Data, "!")
		if len(split) != 2 {
			return nil, fmt.Errorf("excel文件分组坐标格式错误")
		}
		if strings.Contains(split[0], sheetName) {
			arr := reg.FindAllString(split[1], -1)
			if len(arr) != 4 {
				return nil, fmt.Errorf("excel文件分组坐标格式错误, 替换失败")
			}
			startRow, errStartRow := strconv.Atoi(arr[1][1:])
			endRow, errEndRow := strconv.Atoi(arr[3][1:])
			startCol, errStartCol := ParseRowNumber(arr[0][1:])
			endCol, errEndCol := ParseRowNumber(arr[2][1:])
			if errStartRow != nil || errEndRow != nil || errStartCol != nil || errEndCol != nil {
				return nil, fmt.Errorf("excel文件分组坐标格式错误,解析失败")
			}
			centerRow := (startRow + endRow) / 2
			centerCol := (startCol + endCol) / 2

			defined := &XlsxDefinedNamePositions{
			...
			for _, verify := range ret {
				verified := false
				if Iabs(verify.CenterRow-centerRow)*2 > endRow-startRow+verify.EndRow-verify.StartRow {
					verified = true
				}
				if Iabs(verify.CenterCol-centerCol)*2 > endCol-startCol+verify.EndCol-verify.StartCol {
					verified = true
				}
				if !verified {
					return nil, fmt.Errorf("excel文件分组坐标有交叉")
				}
			}
			ret = append(ret, defined)
		}
	}
	return ret, nil
}

func ParseRowNumber(coordinate string) (int, error) {
	bytes := []byte(strings.ToLower(coordinate))
	rowNumber := 0
	power := 26
	for idx, b := range bytes {
		if b < 'a' || b > 'z' {
			return -1, fmt.Errorf("excel文件分组坐标格式错误")
		}
		rowNumber *= power
		rowNumber += int(bytes[idx] - 'a' + 1)
	}
	return rowNumber, nil
}

func (f *File) AddDefinedNames(title, sheetName string, srow, erow, scol, ecol int) {
	data := sheetName + "!$" + MakeColStr(scol) + "$" + strconv.Itoa(srow)
	if srow != erow || scol != ecol {
		data += ":" + "$" + MakeColStr(ecol) + "$" + strconv.Itoa(erow)
	}
...
func MakeColStr(col int) string {
	ret := []byte{}
	for col >= 0 {
		offset := col % 26
		ret = append(ret, 'A'+byte(offset))
		col = col/26 - 1
	}
