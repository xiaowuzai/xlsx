...
func readHyperLinksFromSheet(sheet *Sheet) []*Hyperlink {
	links := make([]*Hyperlink, 0)
	for rowIdx, row := range sheet.Rows {
		for colIdx, cell := range row.Cells {
			link, valid := GetHyperLink(cell.Formula(), rowIdx, colIdx)
			if valid {
...
func readDataValidationsFromSheet(Worksheet *xlsxWorksheet, file *File, sheet *Sheet) ([]*DataValidation, int) {
	var datavalidations []*DataValidation
	var DataValidationCount, i int
	//var err error
	...
	datavalidations = make([]*DataValidation, DataValidationCount)

	if Worksheet.DataValidations != nil {
		// Columns can apply to a range, for convenience we expand the
		// ranges out into individual column definitions.
		for _, rawval := range Worksheet.DataValidations.DataValidation {
			datavalidation := &DataValidation{
				Type:             rawval.Type,
				AllowBlank:       rawval.AllowBlank,
				ShowInputMessage: rawval.ShowInputMessage,
				ShowErrorMessage: rawval.ShowErrorMessage,
				Sqref:            rawval.Sqref,
				Formula1:         rawval.Formula1,
				Formula2:         rawval.Formula2}
			datavalidation.DecryptFormula()
			datavalidations[i] = datavalidation

			i++
		}
	}

	return datavalidations, DataValidationCount
}
...
func readSheetFromFile(sc chan *indexedSheet, index int, rsheet xlsxSheet, fi *File, sheetXMLMap map[string]string, rowLimit int) (errRes error) {
	...
	sheet.DataValidations, sheet.DataValidationCount = readDataValidationsFromSheet(worksheet, fi, sheet)
	sheet.Hyperlinks = readHyperLinksFromSheet(sheet)
	result.Sheet = sheet
	sc <- result
	return nil
}
...
func readStylesFromZipFile(f *zip.File, theme *theme) (*xlsxStyleSheet, error) {
	...
	var fonts []xlsxFont
	for _, font := range style.Fonts.Font {
		font.Sz = xlsxVal{Val: strings.Split(font.Sz.Val, ".")[0]}
		fonts = append(fonts, font)
	}
	style.Fonts.Font = fonts
	return style, nil
}
